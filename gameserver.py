                ...
                if game.match_over:
                    if data_str == "get":
                        ...
                        if "get" not in data_str:
                            ...
                            print(f"[server] after play: moves={game.moves} p1Went={game.p1Went} p2Went={game.p2Went}")
            ...
            if should_reset:
                ...
                with games_lock:
                    if gameId in games:
                        game = games[gameId]
                        safe_broadcast(gameId)
                        print(f"[server] re-broadcasted result state: moves={game.moves} winner={game.winner()}")
                        ...
                        if game.is_match_over():
                            ...
                            game.resetRound()
                            safe_broadcast(gameId)
                            print(f"[server] reset round for game {gameId} and broadcasted reset state: moves={game.moves}")
                            with reset_lock:
                                if gameId in reset_pending:
                                    del reset_pending[gameId]
